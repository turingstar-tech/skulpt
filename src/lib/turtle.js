	...
	function generateTurtleModule(_target) {
		...
		(function (proto) {
			...
			proto.canMove = function (value) {
				if (!this._target || !this._target.hitTest) {
					return false;
				}
		...
		(function (proto) {
			...
			proto.rotate = function (startAngle, delta, isCircle) {
				var self = this;
				return rotate(this, startAngle, delta, isCircle).then(function (
					heading
				) {
			...
			proto.reset = function () {
				...
				this._bufferSize =
					typeof _config.bufferSize === "number" ? _config.bufferSize : 0;
			...
			proto.$degrees = function (fullCircle) {
				fullCircle =
					typeof fullCircle === "number" ? Math.abs(fullCircle) : 360;
			...
			proto.$towards = function (x, y) {
				var coords = getCoordinates(x, y),
					radians =
						Math.PI + Math.atan2(this._y - coords.y, this._x - coords.x),
			...
			proto.$setundobuffer = function (size) {
				this._bufferSize =
					typeof size === "number" ? Math.min(Math.abs(size), 1000) : 0;
			};
			...
			function circleRotate(turtle, angle, radians) {
				return function () {
					return turtle.addUpdate(undefined, false, {
						angle: angle,
						radians: radians,
					});
				};
			...
			proto.$circle = function (radius, extent, steps) {
				...
				if (steps === undefined) {
					...
					steps =
						1 + ((Math.min(11 + Math.abs(radius * scale) / 6, 59) * frac) | 0);
				}
				w = extent / steps;
				w2 = 0.5 * w;
				...
				if (typeof speed !== "number") {
					if (typeof speed === "string") {
						...
						throw new Sk.builtin.TypeError(
							"speed string expected one of " + valid_speeds
						);
					}
			...
			proto.$clone = function () {
				var newTurtleInstance = Sk.misceval.callsimOrSuspendArray(
					_module.Turtle
				);
		...
		(function (proto) {
			...
			proto.setUpWorld = function (llx, lly, urx, ury) {
				...
				world.lineScale = Math.min(
					Math.abs(world.xScale),
					Math.abs(world.yScale)
				);
			};
			...
			proto.$setup = function (width, height, startX, startY) {
				...
				this._xOffset =
					startX !== undefined && !isNaN(parseInt(startX))
						? parseInt(startX)
						: 0;

				this._yOffset =
					startY !== undefined && !isNaN(parseInt(startY))
						? parseInt(startY)
						: 0;

				if (this._mode === "world") {
					return this._setworldcoordinates(
						this.llx,
						this.lly,
						this.urx,
						this.ury
			...
			proto._createKeyDownListener = function () {
				...
				this._keyDownListener = function (e) {
					...
					for (key in self._keyListeners) {
						inKeyMap =
							key.length > 1 && KEY_MAP[code] && KEY_MAP[code].test(key);
			...
			proto.$title = function (title) {
				...
				document.getElementById("headerText_id").innerText = title;
			};
			...
			proto.$numinput = function (title, prompt, defaultValue, minval, maxval) {
				return Sk.turtle_numinput(title, prompt, defaultValue, minval, maxval);
			};
			/* Sk.numinput return a Sk builtin type */
			proto.$numinput.minArgs = 2;
			proto.$numinput.isSk = true;
			proto.$numinput.co_varnames = [
				"title",
				"prompt",
				"defaultValue",
				"minval",
		...
		function pushUndo(turtle) {
			...
			undoState = {};
			properties =
				"x y angle radians color fill down filling shown shape size".split(" ");
			...
			return turtle.addUpdate(function () {
				undoState.fillBuffer = this.fillBuffer
					? this.fillBuffer.slice()
					: undefined;
		...
		var textMeasuringContext = document
		...
		_module.Turtle = Sk.misceval.buildClass(
			_module,
			TurtleWrapper,
			"Turtle",
			[]
		);
		_module.Screen = Sk.misceval.buildClass(
			_module,
			ScreenWrapper,
			"Screen",
			[]
