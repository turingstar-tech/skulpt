...
var $builtinmodule = function (name) {
	...
	function timeZoneName(date) {
		...
		if (Sk.global.navigator != null) {
			language =
				Sk.global.navigator.userLanguage || Sk.global.navigator.language;
		}
	...
	function timeZoneNames() {
		...
		if (dst(jan)) {
			return [
				new Sk.builtin.str(timeZoneName(jul)),
				new Sk.builtin.str(timeZoneName(jan)),
			];
		} else {
			return [
				new Sk.builtin.str(timeZoneName(jan)),
	...
	function date_to_struct_time(date, utc) {
		...
		if (utc) {
			...
			tm_info = [
				new Sk.builtin.str(tm_zone),
				new Sk.builtin.int_(offset * 3600),
			];
		}
    ...
    */
	...
	function strftime_f(format, t) {
		...
		return Sk.ffi.remapToPy(
			Sk.global.strftime(jsFormat, new Date(mktime_f(t).v * 1000))
		);
	}
	...
	function strptime_f(...args) {
		...
		if (_strptime_time === null) {
			return Sk.misceval.chain(
				...
				(s_mod) => {
					_strptime_time = s_mod.tp$getattr(
						new Sk.builtin.str("_strptime_time")
					);
